...
import type { ExtractionRequest, ExtractionResult } from './peel'
...
export async function runExtraction(request: ExtractionRequest): Promise<ExtractionResult> {
  ...
  try {
    switch (request.mode) {
      case 'javascript':
        return renderExtractionValue(await runJavaScriptQuery(query, request.data))
      case 'jsonpath':
...
async function runJavaScriptQuery(query: string, data: unknown): Promise<unknown> {
  const immutableData = deepFreeze(structuredClone(data))
  const evaluator = new Function('data', '"use strict"; return (' + query + ');')
