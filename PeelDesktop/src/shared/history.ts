import type { HistoryRecord, HistoryRecordSeed } from './peel'
...
export function normalizeTitle(title: string | undefined, fallbackDate = new Date()): string {
  const trimmed = title?.trim()
  return trimmed?.length ? trimmed : createDefaultTitle(fallbackDate)
}

export function createHistoryRecord(seed: HistoryRecordSeed = {}): HistoryRecord {
  const now = new Date()
